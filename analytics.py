...
Matrix analysis and reporting. Analyzes synergy/percentage/num_decks matrices
and prints statistics, top cards, symmetry analysis, etc.

For optimization, use optimizer.py instead:
    python optimizer.py --set-size 10 --method ilp
"""
...
def print_top_n(matrix, title, card_names, n=10):
    ...
    row_counts += 1  # epsilon to avoid division by zero
    ...
    col_counts += 1  # epsilon to avoid division by zero
...
def analyze_synergy_symmetry(synergy_matrix):
...
def analyze_matrices(percentage_matrix, synergy_matrix, num_decks_matrix, card_names, card_name_to_id):
    ...
    analyze_synergy_symmetry(synergy_matrix)


def find_good_subset_random(matrix, card_names, set_size=10, num_tries=1000):
    """Random sampling baseline: sample random subsets and keep the best."""
    console.print(Markdown("# Finding Good Subset (Random Sampling Baseline)"))
    best_goodness = -1
    best_subset = None
    ...
    return best_subset, best_goodness


def main():
    percentage_matrix = load_matrix('percentage_matrix.pkl')
    synergy_matrix = load_matrix('synergy_matrix.pkl')
    num_decks_matrix = load_matrix('num_decks_matrix.pkl')
    ...
    # Run optimization using the new fixed optimizer
    from optimizer import optimize
    optimize(synergy_matrix, card_names, set_size=10)
